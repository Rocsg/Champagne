...
mri.start();
	...
	public void readParametersFromHardDisk() {
		...
		this.acquisitionType=AcquisitionType.MRI_T1_SEQ;
	...
	public void computeMask() {
		...
		this.mask=MRI_T1_Seq.areaOfPertinentMRIMapComputation (sourceData[sourceData.length-1],sigmaGaussMapInPixels);
	...
	public void registerEchoes() {
		for(int i=0;i<this.sourceData.length-1;i++) {
			...
			VitiDialogs.notYet("Register Echoes in MRI_T1_SEQ");
			ItkTransform trTemp=null;//BlockMatchingRegistration.testSetupAndRunStandardBlockMatchingWithoutFineParameterization(
	...
	public void registerEchoesITK() {
		for(int i=0;i<this.sourceData.length-1;i++) {
			...
			ImagePlus[] result=null;//manager.runScenarioInterEchoes(sourceData[sourceData.length-1], sourceData[i]);
			System.out.println("Recalage sequence numero "+i+" ok");
