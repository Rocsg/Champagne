...
public class RX extends Acquisition{
	...
	public static void run(String arg) {
		String inputDir=VitiDialogs.chooseDirectoryUI("Select input directory for RX 2D slices","Input directory : ");
		String pathToXml=VitiDialogs.openJFileUI("Choose input file with Xml data describing voxel sizes","Xml file : ",".xml");
		String outputName=VitiDialogs.getStringUI("Select an output name (suggested model : RX_CEP011_AS1.tif", "Output file name : ", "RX_CEP011_AS1.tif", false);
		String outputDir=VitiDialogs.chooseDirectoryUI("Select output directory for RX Volume","Output directory : ");
		ImagePlus img=run(inputDir,pathToXml,outputName,outputDir);
		IJ.saveAsTiff(img, new File(outputDir,outputName).getAbsolutePath());
	}	

	public static ImagePlus run(String inputDir,String pathToXml,String outputName,String outputDir){
		RX rx=new RX(inputDir,pathToXml,outputName,outputDir);
		return rx.makeItAllWithoutWritingFiles();
	}
		
	public ImagePlus makeItAllWithoutWritingFiles() {
		this.printStartMessage();
		double voxSize=parseVoxSizeFromXML(this.pathToXml,false);
		this.normalizedHyperImage = FolderOpener.open(this.sourcePath, "");

		//Lire / appliquer les parametres
		VitimageUtils.adjustImageCalibration(sourceData[1], new double[] {voxSize, voxSize,voxSize},"mm");

		//La subsampler pour produire la deuxieme image source		
		double factorZ=standardVoxSize/sliceThicknessForRegistration;
		System.out.println("Data reduction along Z. Reduction factor = "+factorZ);
		int zEnd=(int)Math.round(this.normalizedHyperImage.getStackSize()*factorZ);
		int xEnd=(int)Math.round(this.normalizedHyperImage.getWidth()*factorZ);
		int yEnd=(int)Math.round(this.normalizedHyperImage.getHeight()*factorZ);
		System.out.println("Final number of slices : "+zEnd);
		IJ.run(this.normalizedHyperImage, "Scale...", "x="+factorZ+" y="+factorZ+" z="+factorZ+" width="+xEnd+" height="+yEnd+" depth="+zEnd+" interpolation=Bilinear average process create");
		this.normalizedHyperImage=IJ.getImage();
		VitimageUtils.adjustImageCalibration(this.normalizedHyperImage,new double[] {sliceThicknessForRegistration,sliceThicknessForRegistration,sliceThicknessForRegistration},"mm");
		this.normalizedHyperImage.hide();
		this.normalizedHyperImage=VitimageUtils.switchAxis(this.normalizedHyperImage,2 );
		this.normalizedHyperImage=VitimageUtils.switchAxis(this.normalizedHyperImage,1 );
		IJ.run(this.normalizedHyperImage, "Flip Horizontally", "stack");
	...
	public RX(String inputDir,String pathToXml,String outputName,String outputDir) {
		super(AcquisitionType.RX, inputDir,Capillary.HAS_NO_CAPILLARY,SupervisionLevel.AUTONOMOUS);
	...
	public void readParametersFromHardDisk() {
		...
		this.acquisitionType=AcquisitionType.RX;
...
//		VitimageUtils.imageChecking(sourceData[1]);
	...
	public void computeMask() {
		ImagePlus img2=maskForRemovingThinStructuresInIsotropicRXImage(sourceData[0],6000,4);
		img2=VitimageUtils.connexe(img2,1,256, 1,10E8, 6,1, false);
		this.mask=VitimageUtils.thresholdShortImage(img2,1,10E8);
	}
	
	public static ImagePlus maskForRemovingThinStructuresInIsotropicRXImage(ImagePlus imgIn,int threshold,int radiusVoxel) {
