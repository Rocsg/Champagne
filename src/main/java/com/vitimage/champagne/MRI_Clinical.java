...
//			writeMask();
			...
			if(i<3) {
				...
				if(i==1) {
					if(makeRegistration) {
						...
						trTemp=trTemp.getFlattenDenseField(VitimageUtils.Sub222(this.sourceData[0]));
	...
	public void computeMask() {
		...
		this.mask=areaOfPertinentMRIMapComputation (sourceData[sourceData.length-1],sigmaGaussMapInPixels);
	...
	public static ItkTransform registerT1T2(ImagePlus imgT1,ImagePlus imgT2,ItkTransform trInit,int mod) {
		...
		BlockMatchingRegistration bmRegistration=new BlockMatchingRegistration(imgRef,imgMov,Transform3DType.DENSE,MetricType.SQUARED_CORRELATION,
					0,sigma,levelMin,levelMax,nbIterations,viewSlice,null,neighXY,neighXY,neighZ,bSXY,neighXY,bSZ,strideXY,strideXY,strideZ,2);
