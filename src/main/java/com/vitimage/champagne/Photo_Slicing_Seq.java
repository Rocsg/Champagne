...
//		String []specimen= {"CEP020_APO1"};//,
	...
	public void readAndProcessSourceData() {
		...
		sourceData[0]=balanceColors(sourceData[0],25);
...
//		weka.fea
	...
	public void computeMaskForAutoRegistration() {
		...
		ImagePlus maskOutFond=makeWekaSegmentation(this.sourceData[1],"/home/fernandr/Bureau/Traitements/Cep5D/CEP019_S3_Martyr/classifier_fond_ombre_et_rouge.model");
		...
		ImagePlus maskOutTraces=makeWekaSegmentation(this.sourceData[1],"/home/fernandr/Bureau/Traitements/Cep5D/CEP019_S3_Martyr/classifier_traces_decoupe.model");
		...
		ImagePlus maskOutBois=makeWekaSegmentation(this.sourceData[1],"/home/fernandr/Bureau/Traitements/Cep5D/CEP019_S3_Martyr/classifier_bois.model");
	...
	public void computeRoughSegmentation() {
		this.roughSegmentation=makeWekaSegmentation(this.sourceData[1],"/home/fernandr/Bureau/Traitements/Cep5D/Weka_hybride/classifier_v2_structure.model");
...
//		this.roughSegmentationHighRes.hide();
	...
	public void extractWood() {
		...
		ImagePlus segFiltered=MostRepresentedFilter.mostRepresentedFilteringWithRadius(imgSeg,4,true,6,true);
		...
		ImagePlus woodOnlyRGB=maskRGB(imgLow, segWood);
	...
	public void cleanWood() {
	 ...
	 // create structuring element (cube of radius 'radius')
		...
		ImagePlus woodOnlyRGB=maskRGB(imgLow, img);
	...
	public void equalizeWood() {
		...
		ImagePlus equalizedRGB=normalizationSliceRGB(woodOnlyRGB,6,6,1,1,false);
...
//			channels[i]=normalizationSliceByte(channels[i],rayX,rayY,rayZup,rayZdown,statsExcludingBlackPoints);
	...
	public void manualRXregistration() {
		...
		ItkTransform firstRXtransform=VitiDialogs.manualRegistrationIn3D(imgRef,imgMov);
	...
	public ImagePlus produceDebugCompositeImage(ImagePlus imgRX,ImagePlus imgPhoto,int []depthSlices,double[]voxThick,int iteration) {
		...
		ImagePlus[]slicesRX=getImagePlusStackAsImagePlusTab(imgRX);
		ImagePlus[]slicesPhoto=getImagePlusStackAsImagePlusTab(imgPhoto);
		for(int z=0;z<Z;z++) {
	...
	public void registerWithRX() {
		...
		for(int it=0;it<nbIterMax;it++) {
			...
			transRX=ItkTransform.readTransformFromFile(this.sourcePath+slash+"Computed_data"+slash+"1_Registration"+slash+"Transforms2"+slash+"trans_RX_iter_0.txt");
			...
			for(int z=0;z<Z;z++) { 
				...
				for(int zRef=zRefMin;zRef<=zRefMax;zRef++){
					...
					IJ.run(imgRef,"8-bit","");
					Object[]ret=testSetupAndRunStandardBlockMatchingWithoutFineParameterizationPhoto2D(
	...
	public static Object[] testSetupAndRunStandardBlockMatchingWithoutFineParameterizationPhoto2D(ImagePlus imgRef,ImagePlus imgMov,ImagePlus mask,ItkTransform trInit,boolean doCoarser,boolean doThinner,String info) {
		...
		transRet=bmRegistration.runBlockMatching(trInit,false);
	...
	public void refineRXregistration() {
		...
		transRX=registerRXOnSlices(imgSlice,imgRX,transRX,93,false);
	...
	public static ItkTransform registerRXOnSlices(ImagePlus imgSlice,ImagePlus imgRX,ItkTransform transRX,int slice,boolean accurate) {
		...
		BlockMatchingRegistration bmRegistration=new BlockMatchingRegistration(imgSlice,imgRX,Transform3DType.VERSOR,MetricType.SQUARED_CORRELATION,
					0,0,levelMin,levelMax,nbIterations,viewSlice,null,neighXY,neighXY,neighZ,bSXY,bSXY,bSZ,strideXY,strideXY,strideZ,2);
		bmRegistration.displayRegistration=0;
		bmRegistration.displayR2=false;
		transRet=bmRegistration.runBlockMatching(transRX,false);
		bmRegistration.closeLastImages();
		bmRegistration.freeMemory();
		return transRet;
	}
	...
	public void automaticMRIregistration() {
		...
		ImagePlus res=null;
		if(doRigid) {
		transMRI=registerMRIonRX(imgRef,imgMov2,transMRI,180,imgCir);
		...
		transMRIToRXWithField=transMRIToRXWithField.getFlattenDenseField(VitimageUtils.Sub222(imgRef));
	...
	public static ItkTransform registerMRIonRX(ImagePlus imgRefH,ImagePlus imgMovH,ItkTransform transMov,int slice,ImagePlus mask) {
		...
		ItkTransform transRet=null;
		BlockMatchingRegistration bmRegistration=new BlockMatchingRegistration(imgRef,imgMov,Transform3DType.VERSOR,MetricType.SQUARED_CORRELATION,
					0,0,levelMinRig,levelMax,nbIterations,viewSlice,mask,neighXY,neighXY,neighZ,bSXY,bSXY,bSZ,strideXY,strideXY,strideZ,2);
		bmRegistration.displayRegistration=0;
		bmRegistration.displayR2=false;
		transRet=bmRegistration.runBlockMatching(transMov,false);
		bmRegistration.closeLastImages();
		bmRegistration.freeMemory();
		return transRet;
	}
	...
	public void computeDataForExpertise() {
		...
		regsThick[0]=IJ.openImage(
				this.sourcePath+slash+"Computed_data"+slash+"1_Registration"+slash+"Stacks2"+slash+"img_SL_thick_start_iter_"+iterRef+".tif");
		ImagePlus []tempTab=ChannelSplitter.split(regsThin[0]);
		regsThin[6]=tempTab[0];
		regsThin[7]=tempTab[1];
		regsThin[8]=tempTab[2];
		tempTab=ChannelSplitter.split(regsThick[0]);
...
*/
	...
	public ItkTransform registerMRandRXFinal(ImagePlus imgRX,ImagePlus imgMRI,ItkTransform trInit) {
		...
		BlockMatchingRegistration bmRegistration=new BlockMatchingRegistration(imgRef,imgMov,Transform3DType.DENSE,MetricType.SQUARED_CORRELATION,
					0,sigma,levelMin,levelMax,nbIterations,viewSlice,null,neighXY,neighXY,neighZ,bSXY,bSXY,bSZ,strideXY,strideXY,strideZ,2);
		bmRegistration.displayRegistration=0;
		bmRegistration.displayR2=false;
